...
(defn update-data
  ...
  (try
    (let [site (if (some? (:sungrow-token site))
          ...
          solar-data (sungrow/get-data (:sungrow-token site) current-time current-time [(:ps-id site) (:ps-point site)])
          power-to-grid-1 (try (- (Float/parseFloat (last (first (get-in solar-data [(:ps-id site) (:ps-point site)]))))) (catch Exception e nil))
...
(defn provide-solar-data-loop
  [solar->data->regulator tesla->state->solar solar->data->logger log-chan error-chan solar-sites]
  (try
    (loop [last-result {:solar-sites solar-sites}]
      (async/>!! log-chan "[Solar] Working...")
      (let [result (provide-solar-data
                    solar->data->regulator
                    tesla->state->solar
                    solar->data->logger
                    log-chan
                    (:car-state last-result)
                    (:solar-sites last-result))]
